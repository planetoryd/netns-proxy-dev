...
pub trait Fetch<T>: Sized {
    #[maybe(fn fetch(ext: T, sock: &mut Socket) -> Result<Self>)]
    async fn fetch<S: AsyncSocket>(ext: T, sock: &mut S) -> Result<Self>;
}

impl Fetch<()> for NftState {
    #[maybe(fn fetch(ext: (), sock: &mut Socket) -> Result<Self>)]
...
impl Fetch<Table> for PTable {
    #[maybe(fn fetch(ext: Table, sock: &mut Socket) -> Result<Self>)]
...
impl Fetch<Chain> for PChain {
    #[maybe(fn fetch(ext: Chain, sock: &mut Socket) -> Result<Self>)]
...
pub trait Fat {
    #[maybe(fn compose(&self, batch: &mut Batch, sock: &mut Socket) -> Result<()>)]
...
impl NftState {
    #[maybe(fn apply(&self, sock: &mut Socket) -> Result<()>)]
...
#[maybe(fn apply_block_forwad(veth_list: &[&str], sock: &mut Socket) -> Result<()>)]
